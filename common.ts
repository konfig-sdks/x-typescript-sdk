...
import { readableStreamToString, XError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new XError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new XError(e, e.response?.data)
                }
                if (innerError instanceof XError) {
                    // Got "XError" from the above try block
                    throw innerError;
                }
